...
export const createManualReviewSchema = z
  .object({
    type: z.enum(['food', 'table', 'event']),
  ...
  .refine(
    (data) => {
      if (data.type === 'food') return !!data.foodId;
      if (data.type === 'table') return true; // tableBookingId optional per current spec
      if (data.type === 'event') return true;
      return false;
    },
    { message: 'foodId is required when type is food' },
  );
